...
const ListRoom = () => {
    ...
    const listRoom = {
        header: { stt: "STT", roomName: "Tên phòng", cinema: "Thông tin rạp", status: "Trạng thái", action: "Action" },
        rooms: allRoom,
    ...
    const handleGetItems = async (cinemaId) => {
        setLoading(true)
        let resRoom = (user.role === "ADMIN") ? await getRoomeByCinemaApi(cinemaId) : await getAllRoomByManagerApi()
        setLoading(false)
        if (resRoom && resRoom.data && resRoom.data.result) {
            setAllRoom(resRoom.data.result)
        }
    }
